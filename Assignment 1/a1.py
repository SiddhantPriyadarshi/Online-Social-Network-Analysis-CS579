...
def bfs(graph, root, max_depth):
    ...
    que.append(root)
    hop = 0
    node2distances[root]=hop
    node2num_paths[root]=hop+1
    seen = [root]
    while que:
        nPop = que.popleft()
        nodeNeighbor= sorted(graph.neighbors(nPop))
        hop = node2distances[nPop]
        if hop >= max_depth:
            break;
        for node in nodeNeighbor:
            if node not in seen:
                seen.append(node)
                que.append(node)
                node2parents[node] = [nPop]
                node2distances[node] = hop + 1
                node2num_paths[node] = node2num_paths[nPop]
            else:
                lastHop = node2distances[node]
                currentHop = hop + 1
                if(lastHop==currentHop):
                    node2num_paths[node] += node2num_paths[nPop]
                    node2parents[node].append(nPop)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    countNode=Counter()    
    edgeBet = dict()
    for node in sorted(node2distances, key=node2distances.get, reverse=True):
        if node != root:
            nlength = node2num_paths[node]
            if nlength > 1:
                for parentNode in node2parents[node]:
                    finalTuple = tuple(sorted([node, parentNode]))
                    edgeBet[finalTuple]=(float)((countNode[node]+1)/len(node2parents[node]))
                    countNode[parentNode]=(float)(countNode[parentNode]+edgeBet[finalTuple])
            else:
                parentNode = node2parents[node][0]
                finalTuple = tuple(sorted([node,parentNode]))
                edgeBet[finalTuple] = float(countNode[node]+1)
                countNode[parentNode] = float(countNode[parentNode] + edgeBet[finalTuple])
    return edgeBet
    pass
...
def partition_girvan_newman(graph, max_depth):
    ...
    comp = get_components(graphCopy)
    while (True):
        graphCopy.remove_edge(*(result[index][0]))
        comp = get_components(graphCopy)
        if len(comp) > 1:
            break
        elif len(comp) == 1:
...
def get_subgraph(graph, min_degree):
    ...
    return graph.subgraph(outputGraph)
    pass
...
def volume(nodes, graph):
    ...
    for edge in graph.edges():
        if(((edge[0] in nodes) or (edge[1] in nodes)) and (edge not in explored)):
...
def brute_force_norm_cut(graph, max_size):
    ...
    for item in range(1,max_size+1):
        for firstCut in combinations(graph.edges(), item):
            graphCopy.remove_edges_from(firstCut)
            num_components = get_components(graphCopy)     
            if len(num_components) == 2:
                normalCut = norm_cut(num_components[0], num_components[1], graph)
                listOfEdges.append((normalCut,list(firstCut)))
            graphCopy.add_edges_from(firstCut)   
    return listOfEdges
    pass
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        if(len(components)>1):
            ...
            listMaxCut.append((depth,normalCut))
    return listMaxCut
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    listNodes=sorted(graphTemp.neighbors(test_node))
    for item in listNodes[0:n]:
            graphTemp.remove_edge(*(test_node,item))    
    return  graphTemp
    pass
...
def jaccard(graph, node, k):
    ...
    for nodeitem in graph.nodes():
        if nodeitem != node and not graph.has_edge(node, nodeitem):
            newneighbors = set(graph.neighbors(nodeitem))
            jaccard = len(neighbors.intersection(newneighbors))/len(neighbors.union(newneighbors))
            listOfEdges.append(((node,nodeitem),jaccard))
    return sorted(listOfEdges,key=lambda x: -x[1])[0:k]
    pass
...
def evaluate(predicted_edges, graph):
    ...
    ###TODO
    frac=0
    for edge in range(len(predicted_edges)):
        if(graph.has_edge(*(predicted_edges[edge]))):
            frac += 1
    return float(frac/len(predicted_edges))
    pass
